...
# ABOUT YOUR TEXTGRID
# -------------------
# The textgrid needs the following tiers:
#     1. syllable:           an interval tier marking each syllable
#     1. prosodic structure: a point tier marking centre of stressed syllables
#                            with a "*" at least.
#     2. pitch accent:       an interval tier indicating the type of each pitch
#                            accent. If the text begins "N-", "PN-", these will
#                            be identified as the pitch accent type.
#     3. points:             a point tier indicating turning points as per
#                            PoLar annotation.
#     4. levels:             a point tier indicating levels are generated by
#                            PoLaR.
#
# ABOUT THE OUTPUT
# ----------------
...
#                If selected, this will add columns for the times of the star
#                time (stress syllable mid-point) along with and local (and
#                global) F0 means and SDs
...
procedure main
    ...
    # Exit with new table selected.
    selectObject: my_table
...
procedure makePitchAccentTable
    ...
    # Create stars only tier
    selectObject: cur_grid
    temp_star_grid = Extract one tier: star_tier
    num_pts = Get number of points: 1
    for i to num_pts
        cur_point = num_pts - i + 1
        cur_label$ = Get label of point: 1, cur_point
        if not(index(cur_label$, "*"))
            Remove point: 1, cur_point
        endif
    endfor
    star_table = Down to Table: "no", 3, "no", "no"
    removeObject: temp_star_grid
    ...
    for i to num_pas
        ...
        # Get time of starred tone
        selectObject: star_table
        pa_star_t[i] = Get value: i, "tmin"
    endfor
    ...
    for cur_pa to num_pas
        ...
        pa_pt_t1[cur_pa] = Get time of point: pts_tier, pa_pt_check
    ...
    for cur_pa to num_pas
        ...
        f0_mean[cur_pa] = Get mean: pa_s_t[cur_pa] - cur_syl_one_s_t,
        ...
        # Relative time to pt_t[1] = 0
        for cur_pt to num_pts[cur_pa]
            pt_t[cur_pa, cur_pt] -= pa_pt_t1[cur_pa]
        endfor
        pa_star_t[cur_pa] -= pa_pt_t1[cur_pa]
    endfor
    ...
    ... "t_secs", "t_norm_syl", "t_norm_PA", "t_norm_star",
    ... "F0_Hz", "F0_z_score_local", "F0_level"}

    if include_reference_values
        Append column: "star_t"
        Append column: "F0_mean_local"
...
procedure mainUI
    ...
    while not okay
        ...
        beginPause: "TextGrid and Pitch Processing"
            ...
            word: "Prosodic structure tier", prosodic_structure_tier$
            ...
            boolean: "Include reference values", include_reference_values
            comment: "Other"
